defmodule Fset.SchTest do
  ...
  test "#get", root do
    {_, _, root} = put(root, "root", "key", New.object())
    assert get(root, "root[key]") == New.object()

    {_, _, root} = put(root, "root[key]", "arr_key", New.array())
    assert get(root, "root[key][arr_key]") == New.array()
    assert get(root, "root[key][arr_key][]") == New.array()
    assert get(root, "root[key][arr_key][][0]") == %{}

    {_, _, root} = put(root, "root[key][arr_key]", New.string())
    ...
    {_, _, root} = put(root, "root", "union", union)
    assert get(root, "root[union][][1]") == New.array()
  end

  test "#put object to object", root do
    {_, _, root} = put(root, "root", "key", New.object())
  ...
  test "#put array to object", root do
    {_, _, root} = put(root, "root", "key", New.array())
  ...
  test "#put object to array", root do
    {_, _, root} = put(root, "root", "arr_key", New.array())
    {_, _, root} = put(root, "root[arr_key]", New.object())
  ...
  test "#put array to array", root do
    {_, _, root} = put(root, "root", "arr_key", New.array())
    {_, _, root} = put(root, "root[arr_key]", New.array())
  ...
  test "#put string to object", root do
    {_, _, root} = put(root, "root", "key", New.string())
  ...
  test "#put string to array", root do
    {_, _, root} = put(root, "root", "arr_key", New.array())
    {_, _, root} = put(root, "root[arr_key]", New.string())
  ...
  test "#pop_schs object", root do
    {_, _, root} = put(root, "root", "key1", New.object())
    {_, _, root} = put(root, "root", "key2", New.object())
  ...
  test "#put_schs multiple props to empty object", root do
    ...
    {_, _, root} = put_schs(root, "root", raw_schs)
  ...
  test "#put_schs multiple items to empty array", root do
    ...
    {_, _, root} = put_schs(root, "root[arr_key]", raw_schs)
  ...
  test "#rename_key", root do
    {_, _, root} = put(root, "root", "a", New.string())
    {_, _, root} = put(root, "root", "y", New.string())
  ...
  test "#move non-any (any, str) in tuple, should not delete the any", root do
    {_, _, root} = put(root, "root", "tuple", New.array(:hetero))
    {_, _, root} = put(root, "root[tuple]", New.any())
  ...
  test "#move a leaf of 1st level list into 3rd level list", root do
    {_, _, root} = put(root, "root", "arr_key", New.array())
    {_, _, root} = put(root, "root[arr_key]", New.boolean())
    {_, _, root} = put(root, "root[arr_key]", New.string())
    {_, _, root} = put(root, "root[arr_key]", New.null())
    {_, _, root} = put(root, "root[arr_key]", New.array(:hetero))
    {_, _, root} = put(root, "root[arr_key][][3]", New.array(:hetero))
